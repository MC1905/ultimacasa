...
if (!empty($zoek)) {
    $filter .= " AND CONCAT_WS('', StartDatum, Datum, Bod, Status, Straat, Postcode, Plaats) LIKE '%" . htmlspecialchars($zoek, ENT_QUOTES, 'UTF-8') . "%'";
};
...
echo '
 ...
 {    echo                    '<tr>
                                    <td>' . htmlspecialchars($tekoop['StartDatum'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td>' . htmlspecialchars($tekoop['Straat'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td>' . htmlspecialchars($tekoop['Plaats'], ENT_QUOTES, 'UTF-8') . '</td>                                             
                                    <td>' . htmlspecialchars($tekoop['Bod'], ENT_QUOTES, 'UTF-8') . '</td>                                             
                                    <td>' . htmlspecialchars($tekoop['Datum'], ENT_QUOTES, 'UTF-8') . '</td>                                             
                                    <td>' . htmlspecialchars($tekoop['Status'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td class="button-column">
                                         <form action="koopbod.php">                             
                                              <button type="submit" class="action-button" id="bieden" name="bieden" 
                                                      value="' . htmlspecialchars($tekoop['TKID'], ENT_QUOTES, 'UTF-8') . '" title="Een bod op dit huis uitbrengen.">&euro;</button>
                                         </form>
                                    </td>
                                    <td class="button-column">
                                         <button class="action-button" title="Contact opnemen met de verkoper.">
                                              <a href="maakmail.php?RID=' . $relatieid . '&FID=' . $relatieid . '&TID=' . htmlspecialchars($tekoop["RID"], ENT_QUOTES, 'UTF-8') . '">&#x2709;
                                    ...
                                    <td class="button-column">
                                         <form action="koopmin.php">     
                                              <button type="submit" class="action-button" id="wis" name="wis" 
                                                      value="' . htmlspecialchars($tekoop['TKID'], ENT_QUOTES, 'UTF-8') . '" title="Dit huis uit mijn lijst verwijderen.">&nbsp;-&nbsp;</button>
                                         </form>
                                    </td>
                               </tr>';
 }
 ...
 {    echo                    '<tr>
                                    <td>' . htmlspecialchars($verkoop['StartDatum'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td>' . htmlspecialchars($verkoop['Straat'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td>' . htmlspecialchars($verkoop['Plaats'], ENT_QUOTES, 'UTF-8') . '</td>                                             
                                    <td>' . htmlspecialchars($verkoop['HoogsteBod'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td>' . htmlspecialchars($verkoop['Status'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td class="button-column">
                                         <form action="verkoopedit.php">     
                                              <button type="submit" class="action-button" id="HID" name="HID" 
                                                      value="' . htmlspecialchars($verkoop['HID'], ENT_QUOTES, 'UTF-8') . '" title="Deze verkoop wijzigen.">...</button>
                                    ...
                                    <td>
                                         <form action="verkoopmin.php">     
                                              <button type="submit" class="action-button" id="wis" name="wis" 
                                                      value="' . htmlspecialchars($verkoop['HID'], ENT_QUOTES, 'UTF-8') . '" title="Deze verkoop intrekken.">&nbsp;-&nbsp;</button>
                                              <input type="hidden" value="' . $relatieid . '" id="RID" name="RID">
                                         </form>
                                    </td>
 ...
 {    echo                    '<tr>
                                    <td>' . htmlspecialchars($criterium['Criterium'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td>' . htmlspecialchars($criterium['Waarde'], ENT_QUOTES, 'UTF-8') . '</td>
                                    <td class="button-column">                                        
                                         <form action="mijncriteriumedit.php">                             
                                              <button type="submit" class="action-button" id="edit" name="edit" 
                                                      value="' . htmlspecialchars($criterium['CID'], ENT_QUOTES, 'UTF-8') . '" title="Dit criterium wijzigen.">...</button>
                                              <input type="hidden" value="' . $relatieid . '" id="RID" name="RID">
                                         </form>
                                    </td>
                                    <td>
                                         <form action="mijncriteriummin.php">     
                                              <button type="submit" class="action-button" id="wis" name="wis" 
                                                      value="' . htmlspecialchars($criterium['CID'], ENT_QUOTES, 'UTF-8') . '" title="Dit criterium verwijderen.">&nbsp;-&nbsp;</button>
                                              <input type="hidden" value="' . $relatieid . '" id="RID" name="RID">
                                         </form>
                                    </td>
                                    ...
                                    <form action="relatieupd.php" method="GET">
                                         ...
                                         <div class="form-group">
                                              ...
                                              <input type="text" class="form-control" 
                                                     id="Naam" name="Naam"
                                                     value="' . htmlspecialchars($gegevens["Naam"], ENT_QUOTES, 'UTF-8') . '" placeholder="Naam" required>
                                         </div>
                                         <div class="form-group">
                                              <label for="Email">E-mail adres:</label>
                                              <input type="email" class="form-control" 
                                                     id="Email" name="Email"
                                                     value="' . htmlspecialchars($gegevens["Email"], ENT_QUOTES, 'UTF-8') . '" placeholder="E-mail adres" 
                                                     pattern="' . $emailpattern . '"required>
                                         </div>
                                         <div class="form-group">
                                              <label for="Telefoon">Telefoon:</label>
                                              <input type="tel" class="form-control" id="Telefoon" name="Telefoon" 
                                                     value="' . htmlspecialchars($gegevens["Telefoon"], ENT_QUOTES, 'UTF-8') . '" placeholder="Telefoonnummer" 
